(ns toshiba-qkd.main
  "Kotodama WASM entrypoint for the Toshiba_qkd clean-room actor (L4) — Clojure port.
  ...
  facts (`toshiba_qkd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "toshiba_qkd")
(def tier "L4")
...
(def entity-specs
  [{:entity "KeySession" :plural "keysessions" :id-prefix "toshibaq_key"
    :fields [:aliceNodeId :bobNodeId :protocol :status] :required [:protocol :status]
    :coerce {}                       :refs {}}
   {:entity "Node"      :plural "nodes"      :id-prefix "toshibaq_nod"
    :fields [:name :location :role]        :required [:name :location]
    :coerce {}                       :refs {}}
   {:entity "Channel"   :plural "channels"   :id-prefix "toshibaq_cha"
    :fields [:sessionId :medium :lossDb]   :required [:medium :lossDb]
    :coerce {:lossDb :float}         :refs {}}
   {:entity "Key"       :plural "keys"       :id-prefix "toshibaq_key"
    :fields [:sessionId :lengthBits :qber :contentRef] :required [:lengthBits :qber]
    :coerce {:lengthBits :int :qber :float} :refs {}}
   {:entity "Protocol"  :plural "protocols"  :id-prefix "toshibaq_pro"
    :fields [:name :family]                :required [:name :family]
    :coerce {}                       :refs {}}
   {:entity "Alarm"     :plural "alarms"     :id-prefix "toshibaq_ala"
    :fields [:nodeId :type :raisedAt]      :required [:type :raisedAt]
    :coerce {}                       :refs {:nodeId "Node"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"toshiba_qkd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "toshiba_qkd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :toshiba_qkd-compat/ready)
